...
mod test_utils {
    ...
    pub fn create_test_project() -> Result<TempDir> {
        ...
        temp_dir.child("src/main.rs").write_str(r#"
fn main() {
    println!("Hello, world!");
}
"#)?;
        
        temp_dir.child("src/lib.rs").write_str(r#"
...
"#)?;
        ...
        temp_dir.child("Cargo.toml").write_str(r#"
[package]
name = "test-project"
version = "0.1.0"
edition = "2021"
...
"#)?;
        ...
        temp_dir.child("README.md").write_str(r#"
# Test Project
...
"#)?;
        ...
        temp_dir.child("target").create_dir_all()?;
        temp_dir.child("target/debug").create_dir_all()?;
        temp_dir.child("target/debug/test-project").write_str("binary content")?;
        
        temp_dir.child(".git").create_dir_all()?;
        temp_dir.child(".git/config").write_str("[core]\n    repositoryformatversion = 0")?;
        
        temp_dir.child(".gitignore").write_str("target/\n*.tmp\n")?;
        
        // Create a large file for testing
        let large_content = "x".repeat(10000);
        temp_dir.child("large_file.txt").write_str(&large_content)?;
        
        // Create a TypeScript file for ts-compressor testing
        temp_dir.child("app.ts").write_str(r#"
interface User {
    name: string;
    email: string;
}
