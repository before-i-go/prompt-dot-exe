...
fn universal_compress(
    ...
    let compressor = UniversalCompressor::with_config(target_folder.clone(), output_dir.clone(), config)
    ...
    let analyzed = configured.analyze()
        .context("Failed to analyze patterns")?;
    let analysis_duration = analysis_start.elapsed();
    info!(duration_ms = analysis_duration.as_millis(), "Pattern analysis completed");
    
    info!("Starting pipeline phase: Dictionary Building");
    let dict_start = Instant::now();
    let built = analyzed.build_dictionary()
        .context("Failed to build dictionary")?;
    let dict_duration = dict_start.elapsed();
    info!(duration_ms = dict_duration.as_millis(), "Dictionary building completed");
    
    info!("Starting pipeline phase: Pattern Replacement Preparation");
    let prep_start = Instant::now();
    let mut ready_compressor = built.prepare_replacement()
        .context("Failed to prepare pattern replacement")?;
    let prep_duration = prep_start.elapsed();
    info!(duration_ms = prep_duration.as_millis(), "Pattern replacement preparation completed");
    ...
    info!(duration_ms = compression_duration.as_millis(), "Compression completed");
    
    // Generate output file with embedded dictionary
    info!("Generating output file");
    let output_start = Instant::now();
    let output_path = generate_output_file(
        &target_folder,
        &output_dir,
        &ready_compressor,
        &result,
    ...
    info!(duration_ms = output_duration.as_millis(), "Output file generation completed");
...
fn write_compression_statistics(
    ...
    writeln!(file, "Files processed: {}", result.statistics.total_files_processed)?;
    writeln!(file, "Original size: {} bytes", result.statistics.original_total_size.bytes())?;
    writeln!(file, "Compressed size: {} bytes", result.statistics.compressed_total_size.bytes())?;
    ...
    writeln!(file, "Compression ratio: {:.2}%", (1.0 - compression_ratio) * 100.0)?;
    writeln!(file, "Dictionary entries: {}", result.dictionary_size)?;
    writeln!(file, "Pattern replacements: {}", result.patterns_replaced)?;
    writeln!(file, "Processing time: {:?}", result.statistics.processing_time)?;
    writeln!(file)?;
...
fn write_directory_manifest(
    ...
    for entry in WalkDir::new(target_folder) {
        let entry = entry?;
        let relative_path = entry.path().strip_prefix(target_folder)
        ...
        Ok(files) => {
            if files.is_empty() {
                ...
                for file_entry in files {
                    ...
                    writeln!(file, "Original size: {} bytes", file_entry.original_size.bytes())?;
...
fn universal_compress_enhanced(
    ...
    use compression::{CompressionConfig, config::ParallelConfig};
    use std::time::Instant;
...
fn handle_checkpoint_command(action: CheckpointAction) -> Result<()> {
    match action {
        ...
        CheckpointAction::Show { database_path, checkpoint_id } => {
            info!(
                database_path = %database_path.display(),
        ...
        CheckpointAction::Delete { database_path, checkpoint_id } => {
            info!(
                database_path = %database_path.display(),
                checkpoint_id = checkpoint_id,
                "Deleting checkpoint"
        ...
        CheckpointAction::Clean { database_path, keep_count } => {
            info!(
                database_path = %database_path.display(),
...
fn delete_checkpoint(database_path: PathBuf, checkpoint_id: i64) -> Result<()> {
    ...
    println!("Deleting checkpoint {} from {}", checkpoint_id, database_path.display());
...
fn clean_checkpoints(database_path: PathBuf, keep_count: usize) -> Result<()> {
    ...
    println!("Cleaning checkpoints, keeping {} latest from {}", keep_count, database_path.display());
