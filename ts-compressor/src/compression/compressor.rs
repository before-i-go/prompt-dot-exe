...
use crate::compression::{
    ...
    FrequencyAnalyzer, DictionaryBuilder, PatternReplacer,
    FrequencyAnalysis, DictionaryBuilding, PatternReplacement,
};
...
impl UniversalCompressor<InitialState> {
    ...
    pub fn new(target_folder: PathBuf, output_dir: Option<PathBuf>) -> Result<Self, CompressionError> {
        let archiver = CodeArchiver::new(target_folder, output_dir)
            .map_err(|e| CompressionError::config_validation(format!("Failed to create archiver: {}", e)))?;
...
impl UniversalCompressor<ConfiguredState> {
    ...
    pub fn analyze(mut self) -> Result<UniversalCompressor<AnalyzedState>, CompressionError> {
        ...
        for (index, file) in files.iter().enumerate() {
            ...
            self.frequency_analyzer.analyze_content(&file.original_content);
        }
    ...
    fn collect_files_from_archiver(&self) -> Result<Vec<crate::compression::types::FileEntry>, CompressionError> {
            ...
            if entry.file_type().is_file() {
                ...
                if self.is_text_file(path) {
                    match fs::read_to_string(path) {
                        Ok(content) => {
                            let relative_path = path.strip_prefix(self.archiver.target_folder())
                                .unwrap_or(path)
                                .to_path_buf();
...
impl UniversalCompressor<AnalyzedState> {
    ...
    pub fn build_dictionary(mut self) -> Result<UniversalCompressor<DictionaryBuiltState>, CompressionError> {
        ...
        info!(pattern_count = patterns.len(), "Retrieved patterns for dictionary building");
...
impl UniversalCompressor<DictionaryBuiltState> {
    ...
    pub fn prepare_replacement(self) -> Result<UniversalCompressor<ReadyState>, CompressionError> {
        ...
        info!(dictionary_size = dictionary_entries.len(), "Retrieved dictionary entries");
        
        // Convert entries to HashMap for PatternReplacer
        debug!("Converting dictionary entries to HashMap");
        let dictionary: std::collections::HashMap<String, String> = dictionary_entries.into_iter().collect();
...
impl UniversalCompressor<ReadyState> {
    ...
    pub fn get_compressed_files(&self) -> Result<Vec<crate::compression::types::FileEntry>, CompressionError> {
    ...
    pub fn compress(&mut self) -> Result<CompressionResult, CompressionError> {
        ...
        use crate::compression::types::{CompressionResult as TypesCompressionResult, CompressionStatistics, FileSize};
        ...
        let pattern_replacer = self.pattern_replacer.as_ref()
            .ok_or_else(|| {
                error!("Pattern replacer not initialized");
                CompressionError::pattern_replacement(
                    "Pattern replacer not initialized".to_string()
                )
            })?;
        ...
        let final_output = if self.config.enable_zstd_compression {
            ...
            info!(duration_ms = zstd_duration.as_millis(), "Zstd compression completed");
            result
        } else {
    ...
    fn collect_files_from_archiver(&self) -> Result<Vec<crate::compression::types::FileEntry>, CompressionError> {
        ...
        // Use walkdir to traverse the target directory
        for entry in WalkDir::new(self.archiver.target_folder()) {
            let entry = entry.map_err(|e| CompressionError::file_processing(
                "directory traversal",
                &format!("Failed to read directory entry: {}", e)
            ))?;
...
impl TryFrom<UniversalCompressor<DictionaryBuiltState>> for UniversalCompressor<ReadyState> {
    ...
    fn try_from(compressor: UniversalCompressor<DictionaryBuiltState>) -> Result<Self, Self::Error> {
...
mod tests {
    ...
    fn test_pipeline_orchestration_prevents_invalid_transitions() {
        ...
        assert!(true, "Will be updated when typestate pattern is implemented");
    }
    ...
    fn test_trait_object_integration_points() {
        ...
        assert!(true, "Trait object integration basic implementation working");
    }
    ...
    fn test_universal_compressor_configuration_management() {
        ...
        let compressor = UniversalCompressor::with_config(target_folder, None, custom_config).unwrap();
        let config = compressor.config();
    ...
    fn test_zstd_compression_levels() {
        ...
        for level in [1, 3, 9] {
            ...
            let compressor = UniversalCompressor::with_config(target_folder.clone(), None, config).unwrap();
            ...
            let result = ready_compressor.compress();
            assert!(result.is_ok(), "Compression should work with level {}", level);
    ...
    fn test_compression_statistics_with_zstd() {
        ...
        assert!(result.statistics.processing_time.as_millis() >= 0);
    ...
    #[test]
    fn test_end_to_end_compression_workflow_with_zstd() {
        let temp_dir = create_test_directory();
        let target_folder = temp_dir.path().to_path_buf();
        ...
        assert!(compressor.current_state_type().contains("InitialState"));
        assert!(compressor.config().enable_zstd_compression); // Default is true
        ...
        // Perform compression with zstd
        let result = ready.compress();
        assert!(result.is_ok());
        ...
        assert!(compression_result.statistics.total_files_processed > 0);
        assert!(compression_result.statistics.dictionary_entries > 0);
        assert!(compression_result.dictionary_size > 0);
        assert!(compression_result.patterns_replaced > 0);
        
        // Verify compression actually occurred
        assert!(compression_result.statistics.original_total_size.bytes() > 0);
        assert!(compression_result.statistics.compressed_total_size.bytes() > 0);
    }
}