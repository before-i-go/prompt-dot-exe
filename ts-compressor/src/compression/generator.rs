...
use crate::compression::{TokenGeneration, CompressionError};
use std::fmt;
...
impl HexToken {
    ...
    fn is_valid_hex_token(token: &str) -> bool {
        ...
        token.len() == 5 && 
        token.starts_with('T') &&
        token.chars().skip(1).all(|c| c.is_ascii_hexdigit() && (c.is_ascii_uppercase() || c.is_ascii_digit()))
    }
}
...
impl HexTokenGenerator {
    ...
    pub fn with_max_tokens(max_tokens: u32) -> Result<Self, CompressionError> {
        if max_tokens == 0 {
            return Err(CompressionError::config_validation("Max tokens must be greater than 0"));
        }
        if max_tokens > Self::MAX_TOKENS {
            return Err(CompressionError::config_validation(
                format!("Max tokens cannot exceed {}", Self::MAX_TOKENS)
            ));
        }
...
mod tests {
    ...
    fn test_token_overflow_handling() {
        ...
        assert!(matches!(generator.next_token(), Err(CompressionError::TokenOverflow)));
    }
