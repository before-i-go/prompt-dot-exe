...
impl FrequencyAnalyzer {
    ...
    pub fn new(min_length: usize, min_frequency: usize) -> Self {
        Self {
            ...
            max_patterns: 50_000, // Limit to 50k patterns to prevent memory explosion
        }
    }
}
        ...
        for window_size in self.min_pattern_length..=content.len().min(50) {
            ...
            if self.pattern_frequencies.len() >= self.max_patterns {
                warn!(
                    ...
                    current_patterns = self.pattern_frequencies.len(),
                    "Pattern limit reached, stopping pattern extraction"
                );
                break;
            }
            ...
            for window in content.as_bytes().windows(window_size) {
                if let Ok(pattern) = std::str::from_utf8(window) {
                    ...
                    if pattern.trim().is_empty()
                        || pattern
                            .chars()
                            .all(|c| c == pattern.chars().next().unwrap())
                    {
                        patterns_skipped += 1;
                        continue;
                    }
                    ...
                    let alphanumeric_count =
                        pattern.chars().filter(|c| c.is_alphanumeric()).count();
                    if alphanumeric_count < pattern.len() / 2 {
                        patterns_skipped += 1;
                        continue;
                    }

                    // Check pattern limit before adding new patterns
...
mod tests {
    ...
    fn test_analyze_content_basic() {
        ...
        assert!(patterns
            .iter()
            .any(|(pattern, _)| pattern.contains("function")));
    }
    ...
    fn test_frequency_threshold_filtering() {
        ...
        let _patterns = analyzer.get_frequent_patterns();
    ...
    fn test_get_frequent_patterns_sorting() {
        ...
        analyzer
            .pattern_frequencies
            .insert("high_freq".to_string(), 10);
        analyzer
            .pattern_frequencies
            .insert("med_freq".to_string(), 5);
        analyzer
            .pattern_frequencies
            .insert("low_freq".to_string(), 2);
        analyzer
            .pattern_frequencies
            .insert("too_low".to_string(), 1); // Below threshold
