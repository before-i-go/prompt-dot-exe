...
use crate::compression::{
    CompressionError, DictionaryBuilding, HexTokenGenerator, TokenGeneration,
};
...
impl DictionaryBuilding for DictionaryBuilder {
    ...
    fn build_dictionary(&mut self, patterns: Vec<(String, usize)>) -> Result<(), CompressionError> {
        ...
        for (pattern, frequency) in sorted_patterns {
            ...
            let token = self.token_generator.next_token().map_err(|e| {
                error!(
                    pattern = %pattern,
                    error = %e,
                    "Failed to generate token"
                );
                CompressionError::dictionary_build(format!(
                    "Failed to generate token for pattern '{}': {}",
                    pattern, e
                ))
            })?;
    ...
    fn validate_dictionary(&self) -> Result<(), CompressionError> {
        ...
        for (pattern, token) in &self.dictionary {
            match self.reverse_dictionary.get(token) {
                ...
                None => {
                    return Err(CompressionError::dictionary_build(format!(
                        "Missing reverse mapping for token '{}'",
                        token
                    )));
                }
            }
...
mod tests {
    ...
    fn test_build_dictionary_basic() {
        ...
        assert_eq!(
            builder.get_replacement_token("function"),
            Some(&"T0000".to_string())
        );
        assert_eq!(
            builder.get_replacement_token("const"),
            Some(&"T0001".to_string())
        );
        assert_eq!(
            builder.get_replacement_token("return"),
            Some(&"T0002".to_string())
        );
    }
    ...
    fn test_build_dictionary_frequency_priority() {
        ...
        assert_eq!(
            builder.get_replacement_token("high_freq"),
            Some(&"T0000".to_string())
        );
        assert_eq!(
            builder.get_replacement_token("med_freq"),
            Some(&"T0001".to_string())
        );
        assert_eq!(
            builder.get_replacement_token("low_freq"),
            Some(&"T0002".to_string())
        );
    }
    ...
    fn test_build_dictionary_duplicate_patterns() {
        let mut builder = DictionaryBuilder::new();
        let patterns = vec![("duplicate".to_string(), 5), ("duplicate".to_string(), 3)];
    ...
    fn test_bidirectional_mapping() {
        let mut builder = DictionaryBuilder::new();
        let patterns = vec![("test_pattern".to_string(), 5)];
    ...
    fn test_clear_dictionary() {
        let mut builder = DictionaryBuilder::new();
        let patterns = vec![("pattern".to_string(), 5)];
        ...
        assert_eq!(
            builder.get_replacement_token("new_pattern"),
            Some(&"T0000".to_string())
        );
    }
    ...
    fn test_try_from_patterns() {
        let patterns = vec![("pattern1".to_string(), 5), ("pattern2".to_string(), 3)];
    ...
    fn test_token_overflow_handling() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Failed to generate token"));
    }
    ...
    fn test_with_custom_token_generator() {
        ...
        assert_eq!(
            builder.get_replacement_token("test"),
            Some(&"T0000".to_string())
        );
    }
}
